...
// RegenerateSessionTitle generates a fresh title for the session from its
// first prompt, leaving the current title in place if that fails.
func (b *Backend) RegenerateSessionTitle(ctx context.Context, workspaceID, sessionID string) error {
	ws, err := b.GetWorkspace(workspaceID)
	if err != nil {
		return err
	}
